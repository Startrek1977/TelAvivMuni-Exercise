...
Contains the 7 named styles shared by all dark themes. This file serves as a **canonical template only** — it is not loaded at runtime and not merged by any theme dictionary.

Both `GruvboxDark.Styles.xaml` and `AyuDark.Styles.xaml` **inline** these 7 styles directly into their own `ResourceDictionary`, replacing their local copies. This avoids the WPF limitation where `StaticResource` lookups cannot cross assembly boundaries in `MergedDictionaries`.
...
- `Blue.Styles.xaml`
- `Emerald.Styles.xaml`
- `AyuDark.Styles.xaml` (inlined from Dark.Styles.xaml canonical template)
- `GruvboxDark.Styles.xaml` (inlined from Dark.Styles.xaml canonical template)
...
| `TelAvivMuni-Exercise.Themes.Zed.GruvboxDark/Themes/GruvboxDark.Styles.xaml` | Inline 7 styles from Dark.Styles.xaml canonical template, remove old duplicates, rename BrowseButtonStyle and DataBrowserTextBoxStyle, fix DataBrowserBox template |
